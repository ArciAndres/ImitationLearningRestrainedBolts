...
class PygameViewer(_AbstractPygameViewer):
    ...
    def _init_drawables(self) -> Set[PygameDrawable]:
        ...
        result.add(self.state.paddleup)
...
class BreakoutState(object):
    ...
    def to_dict(self) -> Dict:
        ...
        paddleup_x = int(self.paddleup.x) // self.config.resolution_x
    ...
    def step(self, command: Command) -> int:
        ...
        paddleup_rect = pygame.Rect(paddleup.x, paddleup.y, paddleup.width, paddleup.height)
...
# for paddle
        if ball_rect.colliderect(paddleup_rect):
            if self.config.complex_bump:
                dbp = math.fabs(ball.x - (paddleup.x + paddleup.width / 2))
                if dbp < 20:
                    # print 'straight'
                    if (ball.speed_x < -5):
                        ball.speed_x += 2
                ...
                dbp = math.fabs(ball.x - (paddleup.x + 0))
                if dbp < 10:
                    # print 'left'
                    ball.speed_x = -abs(ball.speed_x) - 1
                dbp = math.fabs(ball.x - (paddleup.x + paddleup.width))
                if dbp < 10:
                    # print 'right'
                    ball.speed_x = abs(ball.speed_x) + 1
...
class Breakout(gym.Env, ABC):
    ...
    def __init__(self, breakout_config: Optional[BreakoutConfiguration] = None):
        ...
        self._paddleup_x_space = Discrete(self.config.n_paddleup_x)
